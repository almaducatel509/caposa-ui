...
import {
  ...
  Clock, XCircle, Search, X, CheckCheck,
} from 'lucide-react';
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      ...
      <div className="grid items-center px-5 py-3 border-b border-gray-100 bg-linear-to-r from-[#DDEAD5] to-[#F9F9F6]"
        ...
        {COLS.map(col => (
          <button key={col.field} onClick={() => col.field && toggleSort(col.field)}
        ...
        {!isLoading && sorted.map(tx => {
          ...
          return (
            <div key={tx.id}
              ...
              <div>
                <p className="text-sm font-medium text-gray-800 truncate">{tx.member_name ?? '—'}</p>
                {tx.account_number && (
