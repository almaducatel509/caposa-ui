...
interface WithdrawalExportModalProps {
  open:        boolean;
...
const WithdrawalExportModal: React.FC<WithdrawalExportModalProps> = ({
  open, withdrawals, onClose, onConfirm,
}) => {
  ...
  const handleConfirm = async () => {
    try {
      setIsLoading(true);
      setError(null);
      await onConfirm(withdrawals.map(w => w.id));
      onClose();
    } catch (e: any) {
      ...
      <div className="w-full max-w-lg bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-gray-900">
                Exporter {withdrawals.length} retrait{withdrawals.length > 1 ? 's' : ''}
              </p>
              <p className="text-xs text-gray-500 mt-0.5">
                {withdrawals.length} retrait{withdrawals.length > 1 ? 's' : ''} sélectionné{withdrawals.length > 1 ? 's' : ''}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-5 space-y-4 max-h-[60vh] overflow-y-auto">
          <p className="text-sm text-gray-600">
            Les données de ces retraits seront exportées en CSV pour audit ou rapprochement.
          </p>
          ...
          <div className="rounded-xl border border-[#355C7D]/20 overflow-hidden">
            <div className="flex items-center gap-2 px-4 py-2.5 bg-blue-50">
              ...
              <span className="text-xs font-semibold text-[#355C7D]">
                {withdrawals.length} retrait{withdrawals.length > 1 ? 's' : ''} à exporter
              </span>
            </div>
            <div className="divide-y divide-gray-50">
              {withdrawals.slice(0, 5).map(w => (
                <div key={w.id} className="flex items-center justify-between px-4 py-2.5">
                  <div>
                    <p className="text-xs font-semibold text-gray-800">{w.member_name}</p>
                    <p className="text-xs text-gray-400 font-mono">{w.idCompte}</p>
                  </div>
                  <p className="text-xs font-bold text-red-600">
                    −{formatHTG(w.montantTransaction)}
                  </p>
                </div>
              ))}
              {withdrawals.length > 5 && (
                <div className="px-4 py-2 text-xs text-gray-400">
                  +{withdrawals.length - 5} autres retraits inclus
                </div>
              )}
            </div>
          </div>
...
export default WithdrawalExportModal;