...
import {
  ArrowLeftRight, TrendingUp, CalendarDays,
  Users, ShieldCheck, Clock,
  Building2, Landmark, MoreHorizontal,
} from 'lucide-react';
...
export default function TransferStats({
  totalAmount, transferCount, completedCount, avgAmount,
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3">
        ...
        <KPICard icon={TrendingUp}     label="Nombre virements"   value={transferCount} sub={`${completedCount} complétés`} accent={C.blue} />
          ...
          <ResponsiveContainer width="100%" height={160}>
            <LineChart data={volumeData}>
              ...
              <YAxis stroke="#9CA3AF" fontSize={11} tickLine={false} axisLine={false}
                tickFormatter={v => `${(v / 1000).toFixed(0)}k`} />
              <Tooltip contentStyle={tooltipStyle} formatter={(v: number | undefined) => [formatHTG(v ?? 0), 'Montant']} />
              <Line type="monotone" dataKey="amount" stroke={C.blue} strokeWidth={2.5}
                dot={{ fill: C.blue, r: 3 }} activeDot={{ r: 5 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
