...
interface VaultMovementModalProps {
  isOpen: boolean;
  onClose: () => void;
  movementType: 'in' | 'out';
}
...
type MovementStep = 'form' | 'signatures' | 'confirmation';

const VaultMovementModal: React.FC<VaultMovementModalProps> = ({ isOpen, onClose, movementType }) => {
  const [step, setStep] = useState<MovementStep>('form');
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.performedBy || !formData.verifiedBy) {
      setError('Les deux rôles doivent être assignés');
      return false;
    }

    // Règle métier: performedBy et verifiedBy doivent être différents
    if (formData.performedBy === formData.verifiedBy) {
      setError('❌ La personne qui effectue et celle qui vérifie doivent être différentes');
      return false;
    }
  ...
  const canProceedToConfirmation = () => {
    return signatures.performedBy && signatures.verifiedBy;
  };
  ...
  const handleConfirm = () => {
    ...
    // TODO: Appel API pour enregistrer le mouvement
    console.log('💾 Mouvement coffre enregistré:', {
      ...formData,
      signatures,
  ...
  const headerColor = isEntry 
    ? 'from-[#81C784] to-[#66BB6A]' 
    : 'from-[#ff9800] to-[#f57c00]';
  const IconComponent = isEntry ? BiImport : BiExport;

  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div className={`sticky top-0 bg-gradient-to-r ${headerColor} p-6 rounded-t-2xl`}>
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-2xl font-bold text-white flex items-center gap-3">
                <IconComponent className="text-3xl" />
                {isEntry ? 'Entrée au Coffre' : 'Sortie du Coffre'}
              </h2>
              <p className="text-white/90 mt-1">
                {isEntry 
                  ? 'Enregistrer de l\'argent remis au coffre' 
                  : 'Enregistrer de l\'argent sorti du coffre'}
              </p>
            </div>
            <button
              onClick={handleClose}
          ...
          {step === 'form' && (
            <div className="space-y-6">
              <div>
                ...
                {/* Effectué par (Trésorier) */}
                <div className="mb-4">
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Effectué par (Trésorier) <span className="text-red-500">*</span>
                  </label>
                  <select
                ...
                <div className="mb-4">
                  ...
                  >
                    <option value="">Sélectionner un employé</option>
                    {employees.filter(e => e.role === 'Superviseur' || e.role === 'Gestionnaire').map(emp => (
                      <option key={emp.id} value={emp.id}>
                        {emp.name} ({emp.role})
                      </option>
                    ))}
                ...
                {/* Caissier concerné (optionnel pour les sorties) */}
                {!isEntry && (
                  <div className="mb-4">
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      Caissier concerné (optionnel)
                    </label>
                    <select
                      value={formData.relatedCashier}
                      onChange={(e) => setFormData({ ...formData, relatedCashier: e.target.value })}
                      className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-[#2E7D32] focus:ring-2 focus:ring-[#2E7D32]/20 outline-none transition-colors"
                    >
                      <option value="">Aucun</option>
                      {employees.filter(e => e.role === 'Caissier').map(emp => (
                        <option key={emp.id} value={emp.id}>
                          {emp.name}
                        </option>
                      ))}
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      Indiquer le caissier si c'est un fonds de caisse
                    </p>
                  </div>
                )}
                ...
                <div className="mb-4">
                  ...
                  <textarea
                    ...
                    rows={3}
                    placeholder={isEntry 
                      ? "Ex: Remise de fin de journée caisse principale" 
                      : "Ex: Fonds de caisse pour ouverture matinale"}
                  />
                </div>
                ...
                <div className="p-4 bg-blue-50 border-l-4 border-blue-500 rounded-lg">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="text-sm text-blue-800">
                      ...
                      <p>La personne qui effectue le mouvement et celle qui vérifie doivent être différentes.</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-6">
              <div>
                ...
                <p className="text-sm text-gray-600 mb-6">
                  Montant: <span className="font-bold text-[#2E7D32]">{formatCurrency(formData.amount)}</span>
                  {' • '}
                  {isEntry ? 'Entrée au coffre' : 'Sortie du coffre'}
                </p>

                <div className="space-y-4">
                  {/* Signature 1: Effectué par */}
                  <div className={`p-4 rounded-xl border-2 ${signatures.performedBy ? 'bg-green-50 border-green-500' : 'bg-gray-50 border-gray-300'}`}>
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          {signatures.performedBy && <FaCheckCircle className="text-green-600" />}
                          <p className="font-medium text-gray-900">
                            Effectué par: {getEmployeeName(formData.performedBy)}
                          </p>
                        </div>
                        {signatures.performedBy ? (
                      ...
                      {!signatures.performedBy && (
                        <button
                          onClick={() => handleSign('performedBy')}
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm">
                        <span className="font-medium">Effectué par:</span> {getEmployeeName(formData.performedBy)}
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                      <FaCheckCircle className="text-green-600" />
                    ...
                    {formData.relatedCashier && (
                      <div className="flex items-center gap-2">
                        <span className="text-sm">
                          <span className="font-medium">Caissier concerné:</span> {getEmployeeName(formData.relatedCashier)}
                        </span>
                      </div>
                    )}
                  </div>
...
export default VaultMovementModal;