...
export default function CloseAccountModal({
  ...
  const solde         = account.soldeActuel ?? parseFloat((account as any).balance ?? '0');
  const isAlreadyClosed = account.statutCompte === 'ferme'
    || (account as any).statusAccount === 'fermé';
  ...
  const typeLabel: Record<string, string> = {
    epargne: 'Épargne', cheques: 'Chèques', terme: 'Terme',
  };

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={() => { if (!isClosing) onClose(); }} />

      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg border border-gray-100 overflow-hidden max-h-[90vh] flex flex-col">

        {/* Header */}
        <div className="flex items-start justify-between px-6 pt-5 pb-4 border-b border-red-100 bg-red-50 shrink-0">
          ...
          <button onClick={() => { if (!isClosing) onClose(); }}
        ...
        <div className="px-6 py-5 flex flex-col gap-4 overflow-y-auto flex-1">
          ...
          <div className="flex items-start gap-2.5 p-3 bg-blue-50 border border-blue-100 rounded-xl">
            ...
            <p className="text-xs text-blue-700 leading-relaxed">
              <strong>Note importante :</strong> Conformément aux réglementations bancaires, les données du compte seront conservées à des fins de traçabilité et d'audit. Le compte restera consultable mais ne pourra plus être utilisé.
            </p>
          </div>
          ...
          <div className="bg-gray-50 rounded-xl border border-gray-100 overflow-hidden">
            ...
            {[
              ...
              { label: 'Titulaire',      value: account.member_details?.full_name ?? account.id_membre ?? '—' },
              { label: 'Type',           value: typeLabel[account.typeCompte ?? ''] ?? account.typeCompte ?? '—' },
              { label: 'Solde actuel',   value: `${solde.toLocaleString('fr-FR')} HTG`, danger: solde !== 0 },
              { label: 'Ouvert le',      value: account.dateOuverture
                  ? new Date(account.dateOuverture).toLocaleDateString('fr-FR')
                  : account.created_at
                    ? new Date(account.created_at).toLocaleDateString('fr-FR')
                    : '—' },
              { label: 'Statut actuel',  value: (account as any).statusAccount ?? account.statutCompte ?? '—', chip: true },
            ].map(({ label, value, danger, chip }) => (
              <div key={label} className="flex items-center justify-between px-4 py-2.5 border-b border-gray-50 last:border-0">
                ...
                {chip ? (
                  <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-semibold ${
                    value === 'ouvert' || value === 'actif' ? 'bg-[#DDEAD5] text-[#1B5E20]' :
                    value === 'gelé'  || value === 'suspendu' ? 'bg-blue-50 text-[#355C7D]' :
          ...
          {/* Formulaire — seulement si admissible */}
          {canClose && (
          ...
          {canClose && (
            <div className="flex items-center gap-2 p-3 bg-[#DDEAD5]/50 border border-[#2E7D32]/20 rounded-xl">
              ...
              <p className="text-xs text-[#1B5E20] font-medium">
                Solde à 0 HTG — fermeture autorisée
              </p>
            </div>
          )}
        ...
        <div className="flex items-center justify-end gap-2 px-6 py-4 border-t border-gray-100 bg-gray-50 shrink-0">
          <button onClick={() => { if (!isClosing) onClose(); }} disabled={isClosing}
          ...
          <button onClick={handleSubmit} disabled={isClosing || !canClose}
