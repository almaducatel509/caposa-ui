...
interface PostBulkActionDropdownProps {
  selectedCount: number;
  isOpen:        boolean;
  onToggle:      () => void;
  onAction:      (action: PostBulkAction) => void;
}
...
const ACTIONS: {
  ...
  danger?: boolean;
  section: 'permissions' | 'autre';
}[] = [
  { id: 'edit_permissions', label: 'Modifier permissions', icon: <Pencil    className="w-3.5 h-3.5" />, section: 'permissions' },
  { id: 'export',           label: 'Exporter la sélection', icon: <Download  className="w-3.5 h-3.5" />, section: 'autre'       },
  { id: 'delete',           label: 'Supprimer',             icon: <Trash2    className="w-3.5 h-3.5" />, danger: true, section: 'autre' },
];

const SECTIONS: { id: 'permissions' | 'autre'; label: string }[] = [
  { id: 'permissions', label: 'Permissions' },
  { id: 'autre',       label: 'Autre'       },
];

const PostBulkActionDropdown: React.FC<PostBulkActionDropdownProps> = ({
...
export default PostBulkActionDropdown;